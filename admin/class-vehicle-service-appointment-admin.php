...
class Vehicle_Service_Appointment_Admin {
	 ...
	 * @since 1.0.0
	 */
	public function torque_process_appointment() {
	 ...
	 * @since 1.0.0
	 */
	public function torque_customer_search() {
		check_ajax_referer( 'torque360-customer-search', 'nonce' );
		...
		$search_query = ($search_by == "phone") ? "1".preg_replace('[\D]', '', $search_query) : $search_query;
		...
		if( !empty( $search_query ) ){
			...
			wp_send_json_success( json_decode( wp_remote_retrieve_body( $request) ), 200 );
		}

		wp_send_json_error( 'Unable to search, please check API key!' );
	}
}
