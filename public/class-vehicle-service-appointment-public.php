...
class Vehicle_Service_Appointment_Public {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'torque-select2', plugin_dir_url( __FILE__ ) . 'js/select2.min.js', array( 'jquery' ), $this->version, true );
		wp_enqueue_script( 'torque-mask', plugin_dir_url( __FILE__ ) . 'js/jquery.mask.min.js', array( 'jquery' ), $this->version, true );
		wp_enqueue_script( 'torque-public', plugin_dir_url( __FILE__ ) . 'js/vehicle-service-appointment-public.js', array( 'jquery', 'jquery-ui-datepicker', 'torque-select2', 'torque-mask' ), $this->version, true );

		wp_add_inline_script( 'torque-public', 'const ajax = '.json_encode( array(
			'url' => admin_url( 'admin-ajax.php' ),
			'appointment_nonce' => wp_create_nonce( 'torque360-service-appointment' ),
			'search_nonce' => wp_create_nonce( 'torque360-customer-search' ),
		) ), 'before' );
